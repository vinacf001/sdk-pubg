// BlueprintGeneratedClass FBRBuff_v2_Item_Trap_Ra_Lv3.FBRBuff_v2_Item_Trap_Ra_Lv3_C
// Size: 0x4e0 (Inherited: 0x4d8)
struct AFBRBuff_v2_Item_Trap_Ra_Lv3_C : ATslFBRItemBuff {
	struct USceneComponent* DefaultSceneRoot; // 0x4d8(0x08)

	void UserConstructionScript(); // Function FBRBuff_v2_Item_Trap_Ra_Lv3.FBRBuff_v2_Item_Trap_Ra_Lv3_C.UserConstructionScript // Event|Public|BlueprintCallable|BlueprintEvent // @ game+0x22ddc4
};

