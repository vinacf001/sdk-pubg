// BlueprintGeneratedClass BP_GameplayTracerManager.BP_GameplayTracerManager_C
// Size: 0x460 (Inherited: 0x458)
struct ABP_GameplayTracerManager_C : AGameplayTracerManager {
	struct USceneComponent* DefaultSceneRoot; // 0x458(0x08)

	void UserConstructionScript(); // Function BP_GameplayTracerManager.BP_GameplayTracerManager_C.UserConstructionScript // Event|Public|BlueprintCallable|BlueprintEvent // @ game+0x22c9a0
};

